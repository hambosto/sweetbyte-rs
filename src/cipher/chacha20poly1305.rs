...
impl ChaCha20Poly1305 {
    pub fn new(key: &[u8]) -> Result<Self> {
        UnboundKey::new(&CHACHA20_POLY1305, key).context("invalid chacha20poly1305 key")?;
    ...
    pub fn encrypt(&self, plaintext: &[u8]) -> Result<Vec<u8>> {
        ...
        let unbound = UnboundKey::new(&CHACHA20_POLY1305, &self.key).context("failed to create chacha20poly1305 key")?;
        let mut sealing_key = SealingKey::new(unbound, OneTimeNonce(nonce_bytes));
        let mut buffer = plaintext.to_vec();

        sealing_key.seal_in_place_append_tag(Aad::empty(), &mut buffer).context("failed to encrypt with chacha20poly1305")?;
    ...
    pub fn decrypt(&self, ciphertext: &[u8]) -> Result<Vec<u8>> {
        ...
        let plaintext = opening_key.open_in_place(Aad::empty(), &mut buffer).context("failed to decrypt with chacha20poly1305")?;
