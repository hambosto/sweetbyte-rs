...
use crate::config::{ARGON2_SALT_LEN, BLOCK_SIZE, COMPRESSION_LEVEL, ORIGINAL_COUNT, RECOVERY_COUNT};
...
pub(crate) async fn encrypt(source: &Files, target: &Files, secret: &Secret) -> Result<Metadata> {
    ...
    let engine = Pipeline::new(&keys.primary_key, &keys.secondary_key, Processing::Encryption, COMPRESSION_LEVEL, BLOCK_SIZE, ORIGINAL_COUNT, RECOVERY_COUNT)?;
