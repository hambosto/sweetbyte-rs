...
impl Pipeline {
    pub(crate) fn new(primary_key: &Secret, secondary_key: &Secret, operation: Operation) -> Result<Self> {
        let process = Process::new(primary_key, secondary_key, operation).context("failed to init pipeline")?;
        ...
        W: AsyncWrite + Unpin + Send + 'static,
    {
        ...
        stages.spawn(async move { Executor::new(self.process, parallelism.get()).execute(task_rx, result_tx).await.context("executor task failed") });
