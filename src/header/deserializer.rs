...
impl<'a> Deserializer<'a> {
    ...
    fn read_length_sizes<R: Read>(&self, reader: &mut R) -> Result<HashMap<SectionType, u32>> {
        ...
        reader
            .read_exact(&mut lengths_header)
            .context("failed to read lengths header")?;

        let mut sizes = HashMap::new();
        sizes.insert(
            SectionType::Magic,
            BigEndian::read_u32(&lengths_header[0..4]),
        );
        sizes.insert(
            SectionType::Salt,
            BigEndian::read_u32(&lengths_header[4..8]),
        );
        sizes.insert(
            SectionType::HeaderData,
            BigEndian::read_u32(&lengths_header[8..12]),
        );
        sizes.insert(
            SectionType::Mac,
            BigEndian::read_u32(&lengths_header[12..16]),
        );

        Ok(sizes)
    }

    fn read_and_decode_lengths<R: Read>(
        &self,
        reader: &mut R,
        ...
        for section_type in SECTION_ORDER {
            let size = *length_sizes
                .get(&section_type)
                .context("missing length size")?;
    ...
    fn read_and_decode_data<R: Read>(
        &self,
        reader: &mut R,
        ...
        for section_type in SECTION_ORDER {
            let length = *section_lengths
                .get(&section_type)
                .context("missing section length")?;

            let mut encoded_data = vec![0u8; length as usize];
            reader
                .read_exact(&mut encoded_data)
                .with_context(|| format!("failed to read encoded {:?}", section_type))?;
    ...
    fn deserialize_header_data(&mut self, data: &[u8]) -> Result<()> {
        if data.len() < HEADER_DATA_SIZE {
            bail!(
                "invalid header data size: expected {}, got {}",
                HEADER_DATA_SIZE,
                data.len()
            );
        }
